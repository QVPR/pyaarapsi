...
def find_area_factors(S, match_ind: Optional[Union[float, NDArray]] = None):
    if S.ndim == 1:
        S    = S[:, np.newaxis]
    if match_ind is None:
        match_ind = np.argmin(S, axis=0)
    elif not hasattr(match_ind, '__iter__'):
        match_ind = np.array([match_ind])
    assert isinstance(match_ind, np.ndarray)
    ...
    for q in qry_list:
        dvc         = S[:,q]
        _max_ind    = len(dvc) - 1
        _start      = np.max([0,         match_ind[q] - _len])
        _end        = np.min([_shape[0], match_ind[q] + _len])
        _max        = np.max(dvc)
        _min        = np.min(dvc)
        factor_1[q] = np.sum(_max - dvc[_start:_end]) / np.sum(_max - dvc)
        factor_2[q] = np.sum(dvc[_start:_end]) / (len(dvc)* (_max - _min))
        factor_3[q] = np.sum(_max - dvc[np.max([match_ind[q]-1, 0]):np.min([match_ind[q]-1, _max_ind])]) / np.sum(_max - dvc)
        factor_4[q] = np.sum(_max - dvc[np.max([match_ind[q]-1, 0]):np.min([match_ind[q]-1, _max_ind])]) / (len(dvc)* (_max - _min))
    return factor_1, factor_2, factor_3, factor_4

def find_under_area_factors(S, match_ind: Optional[Union[float, NDArray]] = None):
    if S.ndim == 1:
        S    = S[:, np.newaxis]
    if match_ind is None:
        match_ind = np.argmin(S, axis=0)
    ...
    for q in qry_list:
        ...
        factor_2[q] = np.sum(dvc[_start:_end]) / (len(dvc)* (_max - _min))
        factor_3[q] = np.sum(dvc[np.max([match_ind[q]-1, 0]):np.min([match_ind[q]-1, _max_ind])]) / np.sum(_max - dvc)
        factor_4[q] = np.sum(dvc[np.max([match_ind[q]-1, 0]):np.min([match_ind[q]-1, _max_ind])]) / (len(dvc)* (_max - _min))
    return factor_1, factor_2, factor_3, factor_4
...
# def find_sort_factor(S, match_ind, dists):
#     raise Exception("This is broken, don't use it")
#     if S.ndim == 1:
#         S       = S[:, np.newaxis]
#     qry_list    = np.arange(S.shape[1])
#     factor_1    = np.zeros(qry_list[-1] + 1)
#     for i,q in zip(match_ind, qry_list):
#         dvc         = S[:,q]
...
def find_match_distance(S, match_ind: Optional[Union[float, NDArray]] = None):
    ...
    _match_distances = S[match_ind[_arange], _arange]
...
def find_factors(factors_in, sim_matrix, ref_xy: NDArray, \
                 match_ind: Optional[Union[float,NDArray]] = None, \
                 cutoff=2, init_pos=np.array([0,0]), _all=False, dists=None, norm=False, \
                 return_as_dict=False):
    
    _S = sim_matrix[:, np.newaxis] if sim_matrix.ndim == 1 else sim_matrix
    seq  = (_S - np.min(_S, 0)) / (np.max(_S, 0) - np.min(_S, 0)) if norm else _S
    
    if match_ind is None:
        match_ind = np.argmin(seq, axis=0)
    elif not hasattr(match_ind, '__iter__'):
        match_ind = np.array([match_ind])
    ...
    _factors_in, _factors_out = getFactors(_factors_in, _factors_out, ["lgrad", "lcoef"],                                           find_linear_factors,            S=seq, rXY=ref_xy, mXY=ref_xy[match_ind, :], cutoff=cutoff, _all=_all)
    _factors_in, _factors_out = getFactors(_factors_in, _factors_out, ['area', 'area_norm', 'area_small', 'area_small_norm'],       find_area_factors,              S=seq, match_ind=match_ind,                                 _all=_all)
    _factors_in, _factors_out = getFactors(_factors_in, _factors_out, ['uarea', 'uarea_norm', 'uarea_small', 'uarea_small_norm'],   find_under_area_factors,        S=seq, match_ind=match_ind,                                 _all=_all)
    _factors_in, _factors_out = getFactors(_factors_in, _factors_out, ["dlows", "mlows"],                                           find_peak_factors,              S=seq,                                                      _all=_all)
    # _factors_in, _factors_out = getFactors(_factors_in, _factors_out, ["dposi", "dvari"],                                           find_posi_factors,                     rXY=rXY, mXY=rXY[match_ind, :], init_pos=init_pos,   _all=_all)
    # _factors_in, _factors_out = getFactors(_factors_in, _factors_out, ["dsort"],                                                    find_sort_factor,               S=seq, match_ind=match_ind, dists=dists,                    _all=_all)
    ...
    _factors_in, _factors_out = getFactors(_factors_in, _factors_out, ['md_plain', 'rmdist', 'md_mean', 'md_medi'],                 find_match_distance,            S=seq, match_ind=match_ind,                                 _all=_all)
