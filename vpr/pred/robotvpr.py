...
from pyaarapsi.vpr.pred.vpred_tools import find_each_prediction_metric, find_best_match, \
...
from pyaarapsi.vpr.pred.gradseq_tools import create_gradient_matrix, find_best_match_grad, \
    find_best_match_distances_grad, find_consensus
...
def plot_precision_recall(r, p, ax=None, fig=None):
    '''
    Plot a precision and recall curve
    '''
    if ax is None:
        fig,ax=plt.subplots()
    ax.plot(r,p)
...
class RobotVPR:
    ...
    def set_query(self,query: RobotRun):
        ...
        if self.norm:
            self.sim_matrix, self.sim_ref_mean, self.sim_ref_std  = \
    ...
    def set_actual_frame_match(self, ref_frames=None):
        ...
        if ref_frames == None:
            ...
            self.gt_match = ref_frames  # default is a one-to-one match between ref and qry images
        else:
    ...
    def set_tolerance(self, tolerance, units, verbose=True):
        ...
        #--------
        if verbose:
            print(f'tolerance={self.tolerance} {self.units}')
    #
    ...
    def assess_performance(self, tolerance=1, units='frames', match_found=None, verbose=True):
        ...
        Here, match_found=None is not referring to no matches found, but simply not specifying
        matches found.
        '''
    ...
    def plot_PRcurve(self,ax=None):
        ...
        return plot_precision_recall(r,p,ax)
    #
    def plot_cl_vs_bl_PRcurve(self,y_pred,ax=None,fig=None):
        '''
        Plot Precision-Recall curves for baseline VPR technique (using minimum distance as
        a threshold) vs the closed-loop system (which retains only good points, then applies
    ...
    def setup_G(self,tol=1):
        ...
        self.G_match = find_best_match_grad(self.G)
        self.best_match_G = find_best_match_distances_grad(self.G)
...
class RobotVPR_fromS(RobotVPR):
    ...
    def __init__(self,S,actual_match,description=""):
        ...
        self.best_match_S = find_best_match_distances(self.S)
        self.ALL_TRUE = np.full(self.num_qrys,True,dtype='bool')
        # number of frames between VPR match and actual match:
        self.frame_error = abs(self.best_match-self.gt_match)
        # TODO: rework when matches do not exist for each query
        self.match_exists = self.ALL_TRUE
