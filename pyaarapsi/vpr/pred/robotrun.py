...
def get_feature_array(dataset_in: dict, dataset_params: dict, vpr_descriptor: Optional[str] = None
                    ) -> NDArray:
    '''
    Guarantee safe access of feature array
    '''
    if not len(dataset_params['ft_types']) == 1:
        if vpr_descriptor is None:
            raise ValueError("dataset has multiple feature types, but a ft_type specification"
                                " has not been provided.")
        else:
            assert vpr_descriptor in dataset_params['ft_types']
            return dataset_in[vpr_descriptor]
    else:
...
class RobotRun:
    ...
    def from_dataset_dictionaries(self, dataset_in: RosbagDataset, bag_path: str,
                                  vpr_descriptor: Optional[str], odom_topic: Optional[str],
        ...
        single_feature_array = dataset_in.data_of(descriptor_key=vpr_descriptor, \
                                                  topic_name=img_topic)
        
        self.set_features(single_feature_array, 64)
        pxyw = dataset_in.pxyw_of(topic_name=odom_topic)
        # self.set_xy(np.c_[px, py])
        # self.set_yaw(np.degrees(pw))
        self.set_xy(pxyw[:,0:2])
        self.set_yaw(pxyw[:,2])
        self.set_sample_rate(dataset_in.params.sample_rate)
        self.set_description(bag_path)
        self.set_npz_dictionary(dataset_in.params.save_ready())
        return self

    def from_dataset_processor(self, vprdp: VPRDatasetProcessor, vpr_descriptor: Optional[str],
                               odom_topic: Optional[str], img_topic: Optional[str]):
        '''
        TODO
        '''
        self.from_dataset_dictionaries(dataset_in=vprdp.dataset,
                                       bag_path=vprdp.get_bag_path(), vpr_descriptor=vpr_descriptor,
...
# Add timestamps - need to check timestamp length matches other data, and deal with truncation
