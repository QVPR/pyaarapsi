...
import torch
from torch import zeros_like as t_zeros_like, logsumexp as t_logsumexp, cat as t_cat, \
    tensor as t_tensor, exp as t_exp #pylint: disable=E0611
...
class DINOv2(nn.Module):
    ...
    def forward(self, x):
        ...
        f = x[:, 1:]
        # Reshape to (B, C, H, W)
        f = f.reshape((big_b, big_h // 14, big_w // 14, self.num_channels)).permute(0, 3, 1, 2)
...
def log_sinkhorn_iterations(z: torch.Tensor, log_mu: torch.Tensor, log_nu: torch.Tensor,
    ...
    """
    u, v = t_zeros_like(log_mu), t_zeros_like(log_nu)
    for _ in range(iters):
        u = log_mu - t_logsumexp(z + v.unsqueeze(1), dim=2)
        v = log_nu - t_logsumexp(z + u.unsqueeze(2), dim=1)
...
def log_optimal_transport(scores: torch.Tensor, alpha: torch.Tensor, iters: int) -> torch.Tensor:
    ...
    couplings = t_cat([scores, bins], 1)
    norm = - (ms + ns).log()
    log_mu = t_cat([norm.expand(m), bs.log()[None] + norm])
...
class SALAD(nn.Module):
    ...
    def __init__(self,
        ...
        # Dustbin parameter z
        self.dust_bin = nn.Parameter(t_tensor(1.))
...
def get_backbone(
    ...
    Args:
        ...
        backbone_config (dict, optional): this must contain all the arguments needed to instantiate
                                            the backbone class. Defaults to {}.
...
def get_aggregator(agg_arch='ConvAP', agg_config: Optional[dict] = None):
    ...
    Args:
        ...
        agg_config (dict, optional): this must contain all the arguments needed to instantiate
                                        the aggregator class. Defaults to {}.
...
class VPRModel(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # self.batch_acc = [] # keep track of the % of trivial pairs/triplets at the loss level
