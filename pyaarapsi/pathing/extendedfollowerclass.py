#! /usr/bin/env python3
'''
ExtendedFollowerClass, used in sister package aarapsi_robot_pack
'''
import copy
...
from pyaarapsi.core.helper_tools import p2p_dist_2d, angle_wrap, normalize_angle
from pyaarapsi.core.vars import C_I_RED, C_I_GREEN, C_I_YELLOW, C_I_BLUE, C_I_WHITE, C_RESET, \
    C_CLEAR, C_UP_N
...
from pyaarapsi.pathing.enums import Experiment_Mode, Technique, Return_Stage, Save_Request, \
Point_Shoot_Stage, PS4_Pressed, Command_Mode, HDi, Safety_Mode
from pyaarapsi.pathing.basic import calc_current_ind, calc_current_zone, calc_nearest_zone, \
    calc_yaw_error
from pyaarapsi.pathing.basic_rospy import publish_xyzrpy_pose, publish_xyw_pose, q_from_yaw
from pyaarapsi.pathing.simplefollowerclass import SimpleFollowerClass

class ExtendedFollowerClass(SimpleFollowerClass):
    ...
    def experiment(self):
        ...
        if self.new_goal: # If 2D Nav Goal is used to request a goal
            ...
            if (self.path_sum[self.slam_current_ind] - self.path_sum[self.exp_start_SLAM]) \
                < self.exp_dist:
            ...
            if self.point_and_shoot(start=self.point_shoot_start, ego=self.robot_ego,
                                    point=self.point_shoot_point, shoot=self.point_shoot_shoot):
        ...
        # Else: if the current command mode is set to return-to-nearest-zone-boundary:
            ...
            if self.return_stage == Return_Stage.UNSET:
                if self.saved_index == -1: # No saved zone
                    self.zone_index = calc_nearest_zone(self.zone_indices, self.est_current_ind, \
                                                        self.path_xyws.shape[0])
                else:
