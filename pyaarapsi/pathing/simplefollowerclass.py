#! /usr/bin/env python3
'''
SimpleFollowerClass, used in sister package aarapsi_robot_pack
'''
import sys
import copy
...
from aarapsi_robot_pack.msg import ControllerStateInfo, Label, SpeedCommand

from pyaarapsi.core.ros_tools import LogType, twist2xyw, NodeState, pose2xyw
from pyaarapsi.core.image_transforms import np2compressed, compressed2np
from pyaarapsi.core.helper_tools import format_exception, roll, normalize_angle
...
from pyaarapsi.vpr.classes.datasetloader import DatasetLoader
from pyaarapsi.core.argparse_tools import check_positive_float, check_bool, check_string, \
    check_enum, check_positive_int, check_float, check_positive_or_zero_float
from pyaarapsi.pathing.enums import Reject_Mode, Command_Mode, Safety_Mode
from pyaarapsi.pathing.basic import calc_path_errors

class SimpleFollowerClass(DatasetLoader):
    ...
    def init_vars(self, simple=False):
        ...
        try:
            ...
            self.print(format_exception(), LogType.DEBUG)
            self.exit()
        ...
        while not rospy.is_shutdown():
            try:
                ...
                if self.parameters_ready:
                    raise Exception('Critical failure. ' + format_exception()) from e
                else:
                    self.print('Main loop exception, attempting to handle; waiting for parameters '
                               'to update. Details:\n' + format_exception(), LogType.DEBUG, throttle=5)
                    rospy.sleep(0.5)
