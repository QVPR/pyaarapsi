...
def base_optional_args(parser: ap.ArgumentParser, node_name: str = 'node_name', rate: float = 10.0, 
                       ...
                       order_id: Optional[int] = None) -> ap.ArgumentParser:
...
class Super_ROS_Class:
    ...
    def __init__(self, node_name: str, namespace: str, rate_num: float, anon: bool, log_level: LogType, \
                 order_id: Optional[int] = None, throttle: float = 30, colour: bool = True, debug: bool = True, \
        ...
        if reset:
            _log_level = enum_get(rospy.get_param(self.nodespace + '/log_level', _log_level.name), LogType)
            assert not _log_level is None

        rospy.init_node(node_name, anonymous=anon, log_level=np.ceil(_log_level.value).astype(int), disable_signals=disable_signals)
    ...
    def add_pub(self, topic: str, data_class: Type[genpy.Message], queue_size: int = 1, latch: bool = False, 
                subscriber_listener: Optional[SubscribeListener] = None) -> ROS_Publisher:
        '''
