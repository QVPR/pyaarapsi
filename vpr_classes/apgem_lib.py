...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, self_similarity_radius=None, self_similarity_version=1):
        ...
        if self_similarity_radius:
            raise Exception('Imports missing here, please try avoid using this! Code has been commented out.')
        # if self_similarity_radius:
...
def load_checkpoint(filename, iscuda=False):
    ...
    checkpoint = torch.load(filename, map_location=lambda storage, loc: storage) #, map_location=lambda storage, loc: storage
